...
TransMorph Paper:
Chen, J., Frey, E. C., He, Y., Segars, W. P., Li, Y., & Du, Y. (2021).
...
class TransMorphTVFForward(nn.Module):
    def __init__(self, config, time_steps=8):
        '''
        Multi-resolution TransMorph
        '''
        super(TransMorphTVFForward, self).__init__()
        ...
        for t in range(self.time_steps):
            ...
            self.up3s.append(DecoderBlock(embed_dim, config.reg_head_chan, skip_channels=embed_dim // 2 if if_convskip else 0,
                                   use_batchnorm=False))
...
CONFIGS = {
    ...
    'TransMorph-Huge': configs.get_3DTransMorphHuge_config(),
