...
def main():
    ...
    time_steps = 8
    save_dir = 'TransMorphTVFLDDMM_tsteps_{}_mse_{}_mse_{}_LDDMM_alpha{}gamma{}/'.format(time_steps, weights[0], weights[1], alpha, gamma)
    ...
    model = TransMorph.TransMorphTVFForward(config, time_steps)
    #SWIN_SSL = torch.load('experiments/TransMorph_SSL/' + natsorted(os.listdir('experiments/TransMorph_SSL/'))[-1])['state_dict']
    #print('{} loaded!'.format(natsorted(os.listdir('experiments/TransMorph_SSL/'))[-1]))
    #model.transformer.load_state_dict(SWIN_SSL)
    model.cuda()
    ...
    criterion_reg = losses.LDDMM_reg(alpha=alpha, gamma=gamma)
    best_dsc = 0
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            output, flow, flows = model(x_in)
            def_segs = []
            for i in range(36):
                def_seg = model.spatial_trans(x_seg_oh[:, i:i + 1, ...].float(), flow.float())
                def_segs.append(def_seg)
            def_seg = torch.cat(def_segs, dim=1)
            loss_ncc = criterion_mse(output, y) * weights[0]
            loss_seg = criterion_seg(def_seg.float(), y_seg_oh.float()) * weights[1]
            loss_reg = 0
            for i in range(time_steps):
                loss_reg = loss_reg + criterion_reg(flows[i], y)
            loss = loss_ncc + loss_reg + loss_seg
            ...
            del x_in, loss, def_seg

            y_in = torch.cat((y, x), dim=1)
            output, flow, flows = model(y_in)
            def_segs = []
            for i in range(36):
                def_seg = model.spatial_trans(y_seg_oh[:, i:i + 1, ...].float(), flow.float())
                def_segs.append(def_seg)
            def_seg = torch.cat(def_segs, dim=1)
            loss_ncc = criterion_mse(output, x) * weights[0]
            loss_seg = criterion_seg(def_seg.float(), x_seg_oh.float()) * weights[1]
            loss_reg = 0#criterion_reg(flow, y)#0
            for i in range(time_steps):
                loss_reg = loss_reg + criterion_reg(flows[i], x)
            loss = loss_ncc + loss_reg + loss_seg
            ...
            del y_in, def_seg, y_seg_oh, x_seg_oh
            print('Iter {} of {} loss {:.4f}, Img Sim: {:.6f}, Reg: {:.6f}, Seg: {:.6f}'.format(idx, len(train_loader),
                                                                                                loss.item(),
                                                                                                loss_ncc.item(),
                                                                                                loss_reg.item(),
                                                                                                loss_seg.item()))
